...
# Tabmién podríamos usar pandas
def process_titanic_line(line):
    # Dividir las líneas según la "," para obtener los campos
...
def load_titanic(test_size=.25, feature_skip_tuple=(), random_state=1999):
    ...
    # Quitar el . de home.dest, dividir en comilllas porque algunos campos tienen comas
    ...
    # Hacer esto dos veces es muy ineficiente, pero el fichero es pequeño
    ...
    sss = StratifiedShuffleSplit(n_iter=1, test_size=test_size,
                                 random_state=12)
    # Es una forma extraña de conseguir los índices, pero funciona
    train_idx = None
    test_idx = None
    ...
    vec = DictVectorizer()
    # .toarray() ya que viene en un array disperso
...
def read_sentiment_csv(csv_file, fieldnames=FIELDNAMES, max_count=None,
    ...
    import csv  # pon el import dentro si quieres usar IPython.parallel
    ...
    def file_opener(csv_file):
        try:
            ...
            # Python 2 no tiene el argumento encoding
    ...
    with file_opener(csv_file) as f:
        ...
        for i, d in enumerate(reader):
            if i % n_partitions != partition_id:
                # Saltarnos la entrada si no está en la partición requerida
                continue
